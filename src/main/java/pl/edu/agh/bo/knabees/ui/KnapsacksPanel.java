...
public class KnapsacksPanel extends JPanel implements Clearable {
	private static final org.apache.log4j.Logger LOG = Logger.getLogger(KnapsacksPanel.class);
	...
	private void initilize() {
		...
		openFileButton = new JButton("Load knapsacks");
		openFileButton
				.setToolTipText("File should be a plain text file, containing positive integers separated with whitespace - knapsacks capacies");
		...
		openFileButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == openFileButton) {
					int returnVal = fileChooser.showOpenDialog(KnapsacksPanel.this);
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						for (List<Integer> line : Utils.readNumbers(fileChooser.getSelectedFile())) {
							for (Integer i : line) {
								if (i <= 0) {
									LOG.log(Priority.WARN, "Not possitive interers will be ignored!" + i);
								} else {
		...
		listModel = new javax.swing.DefaultListModel<Integer>();
	...
	public List<Knapsack> getKnapsacks() {
		...
		for (int i = 0; i < listModel.getSize(); ++i) {
			result.add(new Knapsack(listModel.get(i)));
		}
		return result;
	}
}
